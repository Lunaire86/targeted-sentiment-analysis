...
from utils.helpers import flatten, plot_results, y_dict, serialise
...
def baseline(parsed_args: Namespace, paths: PathTracker):
    ...
    # Save predictions so we can load for evaluation if needs be
    pred_path = join(path_to.models, f'{basename}_preds.npy')
    mode = 'wb' if os.path.exists(pred_path) else 'xb'
    with open(pred_path, mode) as f:
